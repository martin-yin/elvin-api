...
func GetSurveyStatisticsData(startTime string, endTime string, monitorId string) (surveyStatisticsResponse response.SurveyStatisticsResponse, err error) {
	...
	err = global.GVA_DB.Model(&model.PageIssue{}).Select("COUNT( DISTINCT id ) as js_error").Where(`from_unixtime(page_js_errors.happen_time / 1000, '%Y-%m-%d %H:%i:%s') between date_format( ? , '%Y-%m-%d %H:%i:%s') and date_format( ?, '%Y-%m-%d %H:%i:%s')  and monitor_id = ?`, startTime, endTime, monitorId).Find(&surveyStatisticsResponse.JsError).Error
...
func GetSurveyPUvData(startTime string, endTime string, monitorId string) (surveyPUvData []response.SurveyPUvData, err error) {
	...
	err = global.GVA_DB.Model(&model.PageView{}).Select("FROM_UNIXTIME( happen_time / 1000, '%H:00') AS time_key, COUNT( DISTINCT user_id ) as uv, COUNT( DISTINCT id ) as pv").Where(sqlWhere, startTime, endTime, monitorId).Group("time_key").Find(&surveyPUvData).Error
	return
}
